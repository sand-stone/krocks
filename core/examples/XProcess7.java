...
public class XProcess7 {
  ...
  public static class EventSource implements Runnable {
    ...
    private void write(String col, Client client, List<byte[]> keys, List<byte[]> values) {
      int retry = 5;
      do {
        try {
          client.put(col, keys, values);
          return;
      ...
      while(true) {
        ...
        try (Client client = new Client(uris[0], events)) {
          ...
          do {
            ...
            String col = evtsbuckets.get(bucket);
            keys.clear();
            values.clear();
  ...
  public static class Query implements Runnable {
    ...
    private int queryEvents(String col) {
      int uid = 0;
      int count = 0;
      int total = 0;
      try (Client eventClient = new Client(uris[0], events)) {
        eventClient.open();
        List<byte[]> keys = new ArrayList<byte[]>();
        Client.Result rsp = eventClient.scanFirst(col, 1000);
        count += rsp.count();
        total += rsp.count();
  ...
  public static void main(String[] args) {
    ...
    try (Client client = new Client(uris[0], events, evtopts())) {
      for(int i = 0; i < 13; i++) {
        evtsbuckets.add("bucket"+i);
      }
      client.open(evtsbuckets, "append");
    }
