...
public class Guid3 {
  ...
  private static void write(Client client, List<byte[]> eventKeys) {
    int batchSize = 1000;
  ...
  private static int count(Client client) {
    int index = 0; int step = 100;
    int count = 0;

    Client.Result rsp = client.scanFirst(1000);
    count += rsp.count();
    //System.out.printf("count %d\n", count);
    if(count > 0) {
      do {
        rsp = client.scanNext(1000);
        //System.out.printf("xcount %d\n", rsp.count());
        count += rsp.count();
      } while(rsp.count() > 0);
    }
    return count;
  ...
  public static void main(String[] args) {
    ...
    try (Client stateClient = new Client("http://localhost:8000/", "acme")) {
      stateClient.open(3);
      write(stateClient, keys);
      int c = count(stateClient);
    ...
    try (Client stateClient = new Client("http://localhost:8000/", "acme")) {
      ...
      stateClient.compact();
      int c = count(stateClient);
      System.out.println("second count: " + c);
      write(stateClient, keys);
      c = count(stateClient);
      System.out.println("inserted count: " + c);
      try {Thread.currentThread().sleep(5000);} catch(Exception ex) {}
      stateClient.compact();
      c = count(stateClient);
      System.out.println("third count: " + c);
    }
    System.exit(0);
  }
}
